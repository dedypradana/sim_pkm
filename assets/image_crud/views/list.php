...
function deskripsi(id,sg){
    $.ajax({
       type : "POST",
       url  : "<?php echo base_url(); ?>admin_content/get_img",
       data : "id="+id+"&uri="+sg,
        ...
        <?php foreach($photos as $photo_num => $photo){?>
            <li id="photos_<?php echo $photo->$primary_key; ?>">
                <div class='photo-box'>
                    ...
                    <div class="clear"></div>
                    <div class="qq-upload-button" onclick="deskripsi('<?php echo $photo->$primary_key; ?>','<?php echo $this->CI->uri->segment(2); ?>');" style="position: relative; overflow: hidden; direction: ltr; width: 100%;margin-top: 5px;">
                        <div>Deskripsi</div>
                    </div>
                    <div class="clear"></div>
                </div>
